...
const MARK = "__dshBreakerHideConsole";
const IMPORT_MARK = "dsh-breaker-hide-console";
const HOOK_MARK = "__dshBreakerChildProcessHook";
...
export function hideConsoleEntrySource() {
  ...
  // The facade lives beside this file as dsh-breaker-child-process-hide.mjs
  // (copied by installHideConsoleIntoBin).
  return `// ${IMPORT_MARK} — auto-installed by dsh-breaker; do not edit
import { pathToFileURL } from "node:url";
import { fileURLToPath } from "node:url";

const HOOK_MARK = "__dshBreakerChildProcessHook";
const MARK = ${JSON.stringify(MARK)};
const facade = pathToFileURL(fileURLToPath(new URL("./dsh-breaker-child-process-hide.mjs", import.meta.url))).href;
...
const IMPORT_LINE = `import "./${IMPORT_MARK}.js"; // [dsh-breaker] hide-console`;
...
export function installHideConsoleIntoBin(aiBase, { fs, path: nodePath } = {}) {
  ...
  const facadeSide = p.join(root, "lib", "dsh-breaker-child-process-hide.mjs");
  ...
  text = text.replace(/^import "\.\/dsh-breaker-hide-console\.js";.*\r?\n/m, "");
  if (/^#!/.test(text)) {
...
/**
 * Remove the hide-console preload from bin.js and delete the side files.
 * Idempotent: a bin.js without the import is left untouched.
 */
export function removeHideConsoleFromBin(aiBase, { fs, path: nodePath } = {}) {
  if (process.platform !== "win32") return "skipped_not_win32";
  const nodeFs = fs ?? requireFs();
  const p = nodePath ?? requirePath();
  const root = resolveDshBinRoot(aiBase, { fs: nodeFs, path: p });
  ...
  let text = nodeFs.readFileSync(binJs, "utf8");
  const had = text.includes(IMPORT_MARK);
  if (had) {
    text = text.replace(/^import "\.\/dsh-breaker-hide-console\.js";.*\r?\n/m, "");
    nodeFs.writeFileSync(binJs, text, "utf8");
  }
  for (const f of [side, facadeSide]) {
    try {
  ...
  return had ? "removed" : "not_present";
}
